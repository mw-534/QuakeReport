...
object QueryUtils {
    ...
    /** Log tag used in logging statements. */
     ...
     *
     * @param requestUrl is the URL that forms the request.
     *
    ...
    fun fetchEarthquakeData(requestUrl: String?): ArrayList<Earthquake> {
        ...
        // Extract relevant fields from JSON response and return a list of earthquakes
     ...
     *
     * @param jsonResponse is the JSON response string to be parsed.
     */
    private fun extractEarthquakes(jsonResponse: String?): ArrayList<Earthquake> {
        ...
        // Try to parse the given JSON response. If there's a problem with the way the JSON
     ...
     *
     * @param strUrl is the string to be converted to an URL object.
     */
    private fun createUrl(strUrl: String?): URL? {
        var url: URL? = null

        // If url string is null or empty return early.
        if (strUrl.isNullOrEmpty()) {
            return url
        }

        // parse the url string into an URL object.
        try {
            url = URL(strUrl)
    ...
    /**
     * Perform a GET request to retrieve a JSON string from a web service.
     *
     * @param url is the URL to be queried.
     *
     * @return Returns the JSON String if successful. Otherwise an empty String.
     */
     ...
     *
     * @param inputStream is the [InputStream] that contains the JSON response.
    ...
    private fun readFromStream(inputStream: InputStream?): String {
        ...
        if (inputStream != null) {
            val sr = InputStreamReader(inputStream, Charset.forName("UTF-8"))
